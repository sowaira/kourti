...
task "deploy:test:restart" => ['deploy:set_staging_app', 'deploy:restart']
...
task "deploy:prod:restart" => ['deploy:set_production_app', 'deploy:restart']
...
task "deploy:prod:off" => ['deploy:set_production_app', 'deploy:off']
task "deploy:prod:on" => ['deploy:set_production_app', 'deploy:on']
...
namespace :deploy do
  ...
  task :log do
    puts '======> LOG'
    puts "heroku logs --app #{APP}"
  end

  task :log_tail do
    puts '======> LOG TAIL'
    puts "heroku logs -t --app #{APP}"
  end

  task :command do
    puts '======> COMMAND'
    puts "heroku run rails c --app #{APP}"
  end
  ...
  task :setup do
    ...
    [PRODUCTION_APP, STAGING_APP].each do |app|
      ...
      heroku "addons:add logentries --app #{app}"
      heroku "addons:add heroku-postgresql --app #{app}"
      heroku "addons:add pgbackups:auto-month --app #{app}"
  ...
  task :off do
    puts '======> OFF: Putting the app into maintenance mode'
    heroku "maintenance:off --app #{APP}"
  end

  task :on do
    puts '======> ON: Taking the app out of maintenance mode ...'
